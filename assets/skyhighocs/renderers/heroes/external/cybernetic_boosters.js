...
function initLeftLegBoosters(renderer, model, color) {
  ...
  var beam = renderer.createResource("BEAM_RENDERER", "skyhighocs:cybernetic_booster");
  if (overrides.hasOwnProperty(color)) {
    beam = renderer.createResource("BEAM_RENDERER", "skyhighocs:" + overrides[color] + "_booster");
  };
...
function initLeftArmBoosters(renderer, model, color) {
  ...
  var obj = {
    render: (entity, renderLayer, isFirstPersonArm) => {
      //Booster
      //Equations
      var data_0 = getHoloBooleans(entity, "holoFlight", "rocketsArms") + entity.getInterpolatedData("fiskheroes:flight_timer")*((Math.sin(((5*Math.min(Math.max(entity.getInterpolatedData("skyhighocs:dyn/rockets_arms_timer"), 0.8), 1.0) - 4) * 2.0 - 1.0) * Math.PI / 2.0) + 1.0) / 2.0);
      var data_1 = getHoloBoolean(entity, "holoBoostFlight") + entity.getInterpolatedData("fiskheroes:flight_boost_timer")
      var boost = data_1;
      var flight = data_0;
      var b = Math.min(Math.max(boost * 3 - 1.25, 0), 1);
