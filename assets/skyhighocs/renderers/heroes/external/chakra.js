...
function initChakraAnimations(renderer) {
  //Aiming
  addAnimationWithData(renderer, "chakra.AIMING", "skyhighocs:chakra_aim", "fiskheroes:aiming_timer")
    .setCondition(entity => !entity.getHeldItem().doesNeedTwoHands() && !entity.getHeldItem().isRifle())
    .priority = 10;
  addAnimationWithData(renderer, "chakra.ENERGY_PROJECTION", "skyhighocs:chakra_aim", "fiskheroes:energy_projection_timer")
    .setCondition(entity => !entity.getHeldItem().doesNeedTwoHands() && !entity.getHeldItem().isRifle())
    .priority = 10;
  //Flight
  addFlightAnimation(renderer, "chakra.BASE_FLIGHT", "skyhighocs:flight/chakra_flight.anim.json");
  addAnimationWithData(renderer, "chakra.LAND", "skyhighocs:chakra_landing", "skyhighocs:dyn/superhero_landing_timer")
    .priority = -8;
  addAnimationWithData(renderer, "chakra.LAND_BOOST", "skyhighocs:chakra_boosting_landing", "skyhighocs:dyn/superhero_boosting_landing_timer")
    .priority = -8;
  addAnimationWithData(renderer, "chakra.ROLL", "skyhighocs:flight/chakra_barrel_roll", "fiskheroes:barrel_roll_timer")
    .priority = 10;
  addFlightIdleAnimation(renderer, "chakra.IDLE", "skyhighocs:flight/chakra_idle");
};
...
function initTelekinesis(renderer, name) {
  bindCloud(renderer, "fiskheroes:telekinesis", "skyhighocs:" + name + "_the_invincible");
};