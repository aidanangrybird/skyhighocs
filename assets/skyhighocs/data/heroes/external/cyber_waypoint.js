function initModule(system) {
  ...
  function getWaypointNameArray(entity) {
    var waypointList = entity.getWornHelmet().nbt().getTagList("waypoints");
  ...
  function addWaypoint(entity, manager, waypointName) {
    ...
    if (!entity.getWornHelmet().nbt().hasKey("waypoints")) {
      var waypoints = manager.newTagList();
      manager.appendTag(waypoints, waypoint);
      manager.setTagList(entity.getWornHelmet().nbt(), "waypoints", waypoints);
      system.systemMessage(entity, "<s>Waypoint created with name: <sh>" + waypointName + "<s>!");
    } else {
      var waypoints = entity.getWornHelmet().nbt().getTagList("waypoints");
  ...
  function removeWaypoint(player, manager, waypointName) {
    var waypoints = player.getWornHelmet().nbt().getTagList("waypoints");
  ...
  function teleportToWaypoint(player, manager, waypointName) {
    ...
    if (waypointIndex < 0) {
      ...
      var waypoint = player.getWornHelmet().nbt().getTagList("waypoints").getCompoundTag(waypointIndex);
