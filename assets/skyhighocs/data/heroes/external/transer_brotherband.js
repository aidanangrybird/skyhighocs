      ...
      if (player.getWornChestplate().nbt().hasKey("brothers")) {
        if (player.getWornChestplate().nbt().getStringList("brothers").tagCount() > 0) {
          ...
          if (typeof chat === "string") {
            ...
            if (chatIndex > -1) {
              manager.setData(player, "skyhighocs:dyn/active_chat", chatIndex);
            } else {
              system.moduleMessage(this, player, "<e>You do not have <eh>" + chat + "<e> as a Brother!");
              return;
            };
          } else {
            if (player.getData("skyhighocs:dyn/active_chat") > (brothersList.length-1)) {
              manager.setData(player, "skyhighocs:dyn/active_chat", 0);
            };
          };
          var brother = brothersList[player.getData("skyhighocs:dyn/active_chat")];
