  ...
  return {
    ...
    keyBinds: function (hero) {
      ...
      hero.addKeyBindFunc("DESYNCHRONIZE_WAVES", (player, manager) => {
        ...
        manager.setData(player, "skyhighocs:dyn/crimson_timer", 0);
        manager.setData(player, "skyhighocs:dyn/crimson", false);
      ...
      hero.addKeyBindFunc("SYNCHRONIZE_WAVES", (player, manager) => {
        if (player.getUUID() == "c4bc5db6-3cf6-44fe-8427-304a7b211bc4") {
          system.shoutMessage(player, "<Lucas Stelar> EM Wave Change! \u00A74Lucas Stelar\u00A7r, On-Air!", 16);
          manager.setData(player, "skyhighocs:dyn/battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/selected_battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/body_temperature", 0.0);
          manager.setData(player, "skyhighocs:dyn/predation_timer", 0);
          ...
          system.shoutMessage(player, "<\u00A74Crimson\u00A7r> Who are you?", 16);
        };
        return true;
      }, "EM Wave Change!", 5);
      ...
      hero.addKeyBindFunc("COLD_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == "c4bc5db6-3cf6-44fe-8427-304a7b211bc4") {
          system.shoutMessage(player, "\u00A7r<\u00A74Crimson\u00A7r> You are too cold for us to EM Wave Change.", 16);
        } else {
          system.shoutMessage(player, "<\u00A74Crimson\u00A7r> Who are you?", 16);
        };
        return true;
      ...
      hero.addKeyBindFunc("HOT_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == "c4bc5db6-3cf6-44fe-8427-304a7b211bc4") {
          system.shoutMessage(player, "\u00A7r<\u00A74Crimson\u00A7r> You are too hot for us to EM Wave Change.", 16);
        } else {
          system.shoutMessage(player, "<\u00A74Crimson\u00A7r> Who are you?", 16);
        };
        return true;
    ...
    canAim: function (entity) {
      return (entity.getHeldItem().isEmpty() || entity.getHeldItem().name() == "fiskheroes:chronos_rifle") && entity.getData("fiskheroes:flight_boost_timer") == 0 && entity.getData("skyhighocs:dyn/battle_card") == 0 && entity.getData("skyhighocs:dyn/crimson_timer") == 0 && entity.getData("skyhighocs:dyn/wave_changing_timer") == 1;
    },
    isKeyBindEnabled: function (entity, keyBind) {
      var result = false;
      if (keyBind == "GRAVITY_MANIPULATION") {
        result = (entity.getData("skyhighocs:dyn/battle_card") > 0 || entity.getData("skyhighocs:dyn/crimson_timer") < 1) && entity.getData("skyhighocs:dyn/wave_changing_timer") == 1;
      };
